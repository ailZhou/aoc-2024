...
function deepSearch(value, dirMap){
  if(memo.has(value)){
...
function robotLookup(dir, dirMap){
  const splits = dir.split("A")
  splits.pop();
  let fixed = splits.map((item) => item + "A");
  return fixed.reduce((sum, value) => sum + deepSearch(value, dirMap));
}
...
function part1(data) {
  ...
  for (var i = 0; i < codes.length; i++) {
    ...
    for (var j = 0; j < 5; j++) {
      robotKeyPad = robotLookup(robotKeyPad, bigMap);
    }
